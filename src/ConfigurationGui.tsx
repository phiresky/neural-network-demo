		...
		return (
			<div className="form-group">
				...
				<div
					className={
						"col-sm-6 " +
						(this.props.isStatic ? "form-control-static" : "")
					}
				>
					{this.props.children}
				</div>
	...
	render() {
		return (
			<BSFormGroup label={this.props.label} id={this.props.id} isStatic>
				<input
					type="checkbox"
					checked={this.props.conf[this.props.id]}
					id={this.props.id}
...
export default class ConfigurationGui extends React.Component<
	Configuration,
	{}
> {
	render() {
		...
		return (
			<div className="form-horizontal">
				<div className="col-sm-6">
					<h4>Display</h4>
					<BSFormGroup
						label="Iterations per click on 'Train'"
						id="iterationsPerClick"
					>
						<input
					...
					<BSFormGroup label="Steps per Second" id="stepsPerSecond">
						<input
							...
							max={1000}
							id="stepsPerSecond"
							value={"" + conf.stepsPerSecond}
							onChange={loadConfig}
						/>
					</BSFormGroup>
					<BSCheckbox
						label="When correct, restart after 5 seconds"
						id="autoRestart"
						onChange={loadConfig}
						conf={conf}
					/>
					{conf.type !== "perceptron" ? (
						<BSCheckbox
							label="Show class propabilities as gradient"
							id="showGradient"
							onChange={loadConfig}
							conf={conf}
						/>
					...
					/>
					<BSCheckbox
						label="Show Train Single button"
						id="showTrainSingleButton"
						onChange={loadConfig}
						conf={conf}
					/>
					<button
				...
				<div className="col-sm-6">
					...
					>
						...
						<input
							...
							step={0.005}
							id="learningRate"
							value={Util.logScale(conf.learningRate) + ""}
							onChange={loadConfig}
						/>
					</BSFormGroup>
					<BSFormGroup id="trainingMethod" label="Training Method">
						...
						>
							{Object.keys(trainingMethods).map(name => (
					...
					{conf.type === "perceptron" ? (
						<div>
							{trainingMethods[conf.trainingMethod]
								.trainSingle ? (
								<BSCheckbox
									label="Animate single data points"
									id="animationTrainSinglePoints"
									onChange={loadConfig}
									conf={conf}
								/>
							) : (
...
> {
	render() {
		...
		return (
			<div>
				{p.name} layer: {p.layer.neuronCount} neurons&nbsp;
				<button
					className="btn btn-xs btn-default"
					onClick={() => p.countChanged(1)}
				>
					+
				</button>
				<button
				...
				{p.layer.activation ? (
					<select
						className="btn btn-xs btn-default activation"
						onChange={e =>
							p.activationChanged(e.currentTarget.value)
						}
						value={p.layer.activation}
					>
						{Object.keys(Net.NonLinearities).map(name => (
							<option key={name} value={name}>
								{name}
							</option>
						))}
	...
	render() {
		...
		return (
			<div>
				...
				<NeuronLayer
					key={-2}
					layer={conf.outputLayer}
					name="Output"
					{...neuronListeners(conf.hiddenLayers.length)}
				/>
			</div>
		);
	}
