...
export default class NetworkVisualization implements Visualization {
	...
	constructor(
		...
		public dataPointIsHighlighted: (p: TrainingData) => boolean
	) {
		...
		this.trafo = new TransformNavigation(
			this.canvas,
			() => this.inputMode === 0 /* move view mode */,
			() => this.onFrame()
		);
		...
		switch (this.netType) {
			...
			case NetType.MultiClass:
				...
				for (const name of this.sim.state.outputLayer.names)
					this.actions.push({
						name: name,
						color: NetworkVisualization.colors.multiClass.bg[i++]
					});
	...
	onFrame() {
		...
		if (separator) {
			...
			if (
				...
				this.sim.lastWeights.length > 0
			) {
				const separator = this.getSeparator(
					Util.toLinearFunction(this.sim.lastWeights[0]
						.weights as any)
				);
				this.drawLine(
					tor.x(0),
					separator.min,
					tor.x(this.canvas.width),
					separator.max,
					"gray"
				);
			}
		...
		if (
			...
			for (const val of this.sim.state.data) {
				...
				this.drawPoint(
					ix,
					iy,
					NetworkVisualization.colors.autoencoder.input
				);
				this.drawPoint(
					ox,
					oy,
					NetworkVisualization.colors.autoencoder.output
				);
			}
		} else {
	...
	drawDataPoint(p: TrainingData) {
		const color =
			this.netType === NetType.BinaryClassify
				...
				: this.netType === NetType.MultiClass
					? NetworkVisualization.colors.multiClass.fg[
							Util.getMaxIndex(p.output)
					  ]
					: null;
		this.drawPoint(
			p.input[0],
	...
	drawArrows() {
		...
		const scale = {
			x: (x: number) =>
				this.trafo.toCanvas.x(x * this.sim.state.arrowScale),
			y: (y: number) =>
				this.trafo.toCanvas.y(y * this.sim.state.arrowScale)
		};
		if (
			this.sim.state.inputLayer.neuronCount !== 2 ||
			this.sim.state.outputLayer.neuronCount !== 1 ||
			this.sim.state.hiddenLayers.length !== 0
		)
			throw Error("conf not valid for arrows");

		const weightVector = (weights: number[]) => ({
			x: scale.x(weights[0]),
			y: scale.y(weights[1])
		});
		if (
			steps[steps.length - 1].weights.some(
				(x, i) => x !== steps[0].weights[i]
			)
		) {
			...
			for (const { weights, dataPoint } of steps) {
				this.ctx.strokeStyle = this.ctx.fillStyle = dataPoint
					? NetworkVisualization.colors.binaryClassify.weightVector[
							dataPoint.output[0]
					  ]
					: "#888";
				Util.drawArrow(
			...
			Util.drawArrow(
				this.ctx,
				{ x: scale.x(0), y: scale.y(0) },
				weightVector(steps[steps.length - 1].weights),
				al,
				aw
			);
		}
	...
	drawPolyBackground({ min, max }: { min: number; max: number }) {
		...
		const upperIsClass1 = +(
			this.sim.net.getOutput([this.trafo.toReal.x(0), min - 1])[0] > 0.5
		);
	...
	drawBackground() {
		...
		for (let x = 0; x < this.canvas.width; x += this.backgroundResolution) {
			for (
				...
				y += this.backgroundResolution
			) {
				const vals = this.sim.net.getOutput([
					this.trafo.toReal.x(x + this.backgroundResolution / 2),
					this.trafo.toReal.y(y + this.backgroundResolution / 2)
				]);
				if (this.sim.state.outputLayer.neuronCount > 2) {
					this.ctx.fillStyle =
						NetworkVisualization.colors.multiClass.bg[
							Util.getMaxIndex(vals)
						];
				} else {
					if (this.sim.state.showGradient) {
						this.ctx.fillStyle = NetworkVisualization.colors.binaryClassify.gradient(
							vals[0]
						);
					} else
						this.ctx.fillStyle =
							NetworkVisualization.colors.binaryClassify.bg[
								+(vals[0] > 0.5)
							];
				}
				this.ctx.fillRect(
	...
	refitData() {
		...
		if (this.sim.state.inputLayer.neuronCount == 2) {
			...
			const scale =
				Math.min(this.canvas.width / w, this.canvas.height / h) *
				fillamount;
			this.trafo.scalex = scale;
			this.trafo.scaley = -scale;
			this.trafo.offsetx =
				(-(bounds.maxx + bounds.minx) / 2) * scale +
				this.canvas.width / 2;
			this.trafo.offsety =
	...
	canvasClicked(evt: MouseEvent) {
		...
		if (this.inputMode === removeMode || evt.button == 2 || evt.shiftKey) {
			...
			this.inputMode < removeMode &&
			this.inputMode > 0 /* move mode */
		) {
			// add data point
			if (this.netType === NetType.AutoEncode) {
				...
				if (this.netType === NetType.MultiClass) {
					output = Util.arrayWithOneAt(
						this.sim.state.outputLayer.neuronCount,
						label
					);
				}
